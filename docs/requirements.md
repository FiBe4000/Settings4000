...
- **R5.3** On Apply, the app must:
  ...
  2. Execute the required reload commands, only for running/installed components: `hyprctl reload`, `eww reload`, `swaync-client -rs`, `kill -SIGUSR1 $(pidof kitty)`, `hyprctl hyprpaper ...`, restart `hypridle` when its config changed.
